...
//
// 🔴 **حدٌّ مُعلَنٌ لا عطلٌ مجهول — لا يُتتبَّع سياقُ الاقتباس:** سلسلةٌ **مفردةٌ خارجياً**
//    تجعل الـbacktick حرفيّاً، **ويُبلَّغ عنها زوراً**. قِيس بالتنفيذ:
//         echo 'A: `echo RAN`'      ⇒ `A: `+'`'+`echo RAN`+'`'+`   **حرفيّ**  ⇒ إنذارٌ كاذب
//         echo "B: '`echo RAN`'"    ⇒ `B: 'RAN'`                   **نُفِّذ**   ⇒ صحيح
//    🔴 **ولا يُسوّى بين الوجهين:** الثاني **يُنفَّذ وهو أخطرُ ما يُصاد هنا**، و«إصلاحٌ»
//       يسوّي بينهما **يفتح الثغرة** بدل أن يغلقها.
//    ⚠️ **ولماذا وُثّق ولم يُغلَق — خلافاً لفجوة الشرطتين:** تتبّعُ الاقتباس يفتح فجواتِ
//       حالاتٍ (متغيّرات · heredoc · وصلُ أسطر) **ويُفارق النسخَ الشقيقةَ فتتباعد بصمت**
//       ⇒ **الثمنُ هنا حقيقيّ**، وهناك كان العدُّ بلا ثمن. **والسؤالُ واحدٌ في الحالتين:
//       أثمنُ إغلاقِها ضجيجٌ أم لا؟ — يُقاس لا يُفترَض.**
//    (‏رفعَته مراجعةُ `claude[bot]` على `SchoolApp-gas` PR #1515، وأُعيد قياسُه هنا.)
//
...
/* 🔴 **مجموعةُ مدخلاتٍ فارغةٍ ليست نتيجةً — تخرج بـ2 لا بـ0.**
 *
 * بغير هذا يصير «صفرُ مطابقة» **بالمصادفة لا بالإنجاز**: مسارٌ تغيّر · مجلدٌ نُقل · نمطُ
 * الامتداد بطَل بعد إعادة هيكلة ⇒ الحارسُ يفقد مدخلاتِه **ويُعلن السلامةَ صادقَ الشكل،
 * وسجلُّه أخضرُ نظيف**. وهو **فشلٌ صامتٌ في أداة الكشف نفسِها** — أسوأُ من غياب الحارس،
 * لأن الغيابَ معلومٌ والخُضرةَ الكاذبةَ تُطمئن.
 *
 * ⚠️ وفحصُ **وجودِ المجلد** أعلاه لا يغني عنه: مجلدٌ قائمٌ خالٍ من `.yml` يمرّ منه.
 * 🟢 وهو تفعيلُ «صفرُ مِجَسٍّ لم يُقَس ليس براءة» **شرطاً في الكود** لا تحذيراً يُقرأ —
 *    والقاعدةُ التي تصير شرطاً تُطبَّق، والتي تبقى تحذيراً تُنسى.
 * (‏كشفَته جلسةُ `YemenSchoolz` عند أوّل نقلٍ لهذا الملفّ — `PR #17` · `58036f1`.) */
if (!files.length) {
  console.error('❌ صفرُ ملفّاتٍ في: ' + dir);
  console.error('   الحارسُ بلا مدخلات ⇒ لا حكمَ له. «صفرُ مطابقة» هنا مصادفةٌ لا نتيجة.');
  process.exit(2);
}
