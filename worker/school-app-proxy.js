...
//
// 🗄️ **تاريخيّ (2026-08-11 → 08-22) — لا يُعمَل به اليوم:** كان تسجيلُ مدرسةٍ جديدة يتطلّب
//    إضافةَ slugها **هنا وفي `_build/schools.public.json` معاً**. **بطَل ذلك** بالسجلّ الحيّ
//    `listPublicSlugsPublic` أدناه ⇒ **المدرسةُ الجديدة لا تُضاف هنا يدوياً.** والبذرةُ تبقى
//    شبكةَ أمانٍ عند سقوط GAS لا قائمةً تُصان (السببُ في الكتلة التالية).
...
export default {
  async fetch(request, env, ctx) {
     ...
     *   · 🧹 **`GAS.pricing` حُذف من الجدول 2026-09-24** بقرار المالك (الحذفُ النهائيّ)، و`/gas/pricing`
     *     ⇒ 410 عبر `_RETIRED_GAS_APPS` (انظر تعليقَ الجدول).
