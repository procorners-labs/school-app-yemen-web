    ...
    if (isHtml) {
      headers.set('Content-Security-Policy-Report-Only', [
        ...
        /* 🟢 **وُسّعت 2026-09-11 بعد ٤٠ دقيقةً من نشر الإبلاغ — والقائمةُ الأصليّةُ كانت ناقصة،
           وهذا بعينه ما وُضع `Report-Only` لأجله.** التقارير الواردة (‏`ev:'csp'`) أعطت
           **٩ انتهاكاتِ `connect-src` و١ `script-src-elem` من مستخدمين حقيقيّين** — كلُّها
           ...
           و**`stats.g.doubleclick.net`** (لا إلى `www.google-analytics.com` وحدَه كما افترضتُ
           من الجرد الساكن)، و**`static.cloudflareinsights.com/beacon.min.js`** يُحقنه Cloudflare
           **على الحافّة فلا يظهر في `frontend/` إطلاقاً**.
