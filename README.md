...
| `/gas/student` | `teacher/` ← **لا `student/`** | ⚠️ **مفطوم**: يُخدَم من نشرة **المعلّم** بمُميِّز `app=student` **بصفةٍ دائمة**. ونشرةُ `student` الخاصّة **حيّةٌ خاملةٌ وليست مسارَ تراجع**: كودُها القديم يقرأ ورقةً محذوفة (‏`health` ⇒ `ok:false` · قِيس 2026-09-24). والمعرّفُ سجلٌّ لا وجهة — 🔴 لا يُنشَر عليها |
| `/gas/cms` | `cms/` | 🟢 حيّ |
...
| `/gas/schedule` | `schedule/` | 🗑️ **متقاعد: `/gas/schedule` يردّ `410`** بلا نداءٍ على Google (`_RETIRED_GAS_APPS`)، والصفحةُ `/schedule/index.html` إعلانُ تقاعدٍ يردّ 200. والمسارُ محجوزٌ في `_RESERVED_TOP_PATHS` عمداً ويحرسه `worker/test-routes.js` |
| `/pricing` | — (مشروعُه متقاعد 2026-09-10) | ↪️ **ليس صفحة: `301` إلى `/#pricing`** (قرار المالك 2026-09-19 — الأسعار في قسم الرئيسية وحدَه). والاسمُ محجوزٌ في `_RESERVED_TOP_PATHS`. أمّا `/gas/pricing` فنشرتُه حيّةٌ خاملةٌ تردّ 200 عبر `GAS[app]`، ويبقى بقاعدة «لا يُسقَط مسارٌ يردّ 200» |
