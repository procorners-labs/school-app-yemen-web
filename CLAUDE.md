...
(~6% من الطلبات). `/gas/<app>` يعيد المحاولة تلقائياً حتى محاولتين (فاصل 700ms بينهما، مهلة 11.5
ثانية للمحاولة الواحدة، ميزانية كلية 24 ثانية — طلبات POST فقط؛ GET يقبل HTML طبيعياً)، ويرجع JSON
خطأ صالح (503) لا HTML خام عند استنفاد المحاولات — يهبط معدّل الفشل الظاهر للمستخدم من ~6% إلى ~0.02%.
...
والفائض ينتظر في طابور FIFO حتى 8 ثوانٍ ثم يُرفض بـ503 نظيف — لا 502 خام أبداً من هذا المسار.
...
- **مفتاح التحكّم:** `env.BULKHEAD_MODE` ∈ `on` (الافتراضي) · `shadow` (يقيس بلا انتظار ولا رفض —
  ...
  إضافته إلى `vars` في `wrangler.jsonc` متابعة مقترَحة (تُمكّن التبديل بلا تعديل كود).
